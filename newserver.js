...
const Tracking = mongoose.model('Tracking_dls', trackingSchema);
...
const fetchData = async (serialNumber,dlsLoadNo='') => {
    ...
    if (existingTracking) {
        ...
        try {
            // fetch it from localhost server.
...
// get luxcross data
const luxcrossData = async (serialNumber,dlsLoadNo='') => {
    const existingTracking = await Tracking.findOne({ SerialNumber: serialNumber });
...
app.post('/api/updates', async (req, res) => {
    const { serialNumber, sensorType, dlsLoadNo } = req.body;
    ...
    try {
        // Determine the sensor type, either from the database or request body
        let sensor_type = await sensorInfo(serialNumber);
        if (!sensor_type) {
            sensor_type = sensorType;
        }
        ...
        if (sensor_type === 'sensitech') {
            ...
            trackingData = await luxcrossData(serialNumber, dlsLoadNo);
            if (!trackingData) {
                return res.status(404).json({ error: 'Tracking data not found for Luxcross' });
            }
            tempData = await getTemperatureluxData(trackingData);
        ...
        // Update the DLSLoadNo if it's provided and the tracking data already exists
        if (dlsLoadNo && trackingData) {
            trackingData.DLSLoadNo = dlsLoadNo;
...
//Edit the load number
app.put('/api/edit', async (req, res) => {
    const { serialNumber, dlsLoadNo } = req.body;

    if (!serialNumber || !dlsLoadNo) {
        return res.status(400).json({ error: 'SerialNumber and DLSLoadNo are required' });
    }

    try {
        const updatedTracking = await Tracking.findOneAndUpdate(
            { SerialNumber: serialNumber },
            { $set: { DLSLoadNo: dlsLoadNo } },
            { new: true } // Return the updated document
        );

        if (updatedTracking) {
            res.status(200).json({ status: 'Success', updatedTracking });
        } else {
